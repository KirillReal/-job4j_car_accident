...
public class AccidentJdbcTemplate {
    ...
    public List<Accident> getAll() {
        return jdbc.query("select a.id, a.name AS a_name, a.text, "
                        + "a.address, a.type_id, t.name AS t_name "
                        + " from accident a left join types t ON "
                        + "a.type_id = t.id",
                (rs, row) -> {
    ...
    public Accident findById(int id) {
        return  jdbc.queryForObject(
                "select * from accident where id = ?", new Object[]{id},
                (rs, row) -> {
                    Accident accident = new Accident();
                    accident.setId(rs.getInt("id"));
                    accident.setText(rs.getString("text"));
    ...
    public List<Rule> getRulesForAccident(int id) {
        return jdbc.query(
                "select a_r.rule_id, r.name from accident_rule a_r"
                        + " join rule r ON r.id = a_r.rule_id where a_r.accident_id= ?",
                new Object[]{id},
                (rs, row) -> {
                    Rule rule = new Rule();
    ...
    public void create(Accident accident) {
        ...
        String msgSql =  "insert into accident (name, text, address, type_id) values (?, ?, ?, ?)";
